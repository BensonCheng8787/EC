...
uint32_t errorSumL, errorSumR, state, dw, speedL, speedR, pwm_setL, pwm_setR, dv,
ki;
int32_t pwm_max = 720;
int32_t pwm_min = 70;

uint16_t targetDist = 50;
float kdp = 1;
...
void Distance(){
    printf("running1\r\n");
    uint16_t curDist = readRanger();
    printf("running2\r\n");
    int8_t dir = 0; // 1 = forward and -1 = backward
    int16_t diffDist = targetDist - curDist;
    if(diffDist>0){//too close
        kdp = kdp*-1;
        dir = -1;
    }
    else if(diffDist<0){//too far
        diffDist = diffDist*-1;
        dir = 1;
    }
    pwm_setL = (kdp*diffDist)/80 * 800;
    pwm_setR = (kdp*diffDist)/80 * 800;
    /*if(dir>0){//forward
    ...
    printf("pwm_setL: %d, pwm_setR: %d ,cur: %d  diff: %d dir: %d \r\n", pwm_setL, pwm_setR, curDist, diffDist, dir);
}
